var pathname = window.location.pathname + "/",
    ...
    table, table2, checkSeleccionarTodasLasDeducciones = $("#checkSeleccionarTodasDeducciones"),
    urlSinElIndex = "";
...
function getDDL() {
    _ajax(null, urlSinElIndex + "/getPeriodos", "GET", data => {
        ...
        let valEditar = $("#valFrecuenciaEnDias").val(),
            valorEstaEnEditar = !(estaEnCrear() > 0);
        valorEstaEnEditar && $("#cpla_FrecuenciaEnDias").append("<option value=0>Selecione una opción...</option>"), $.each(data, (function (i, iter) {
            valorEstaEnEditar ? $("#cpla_FrecuenciaEnDias").append("<option value='" + iter.id + "' " + (iter.id == valEditar ? "selected" : "") + ">" + iter.descripcion + "</option>") : $("#cpla_FrecuenciaEnDias").append("<option value='" + iter.id + "'>" + iter.descripcion + "</option>")
        }))
    }, () => { })
}
var crearEditar = function (edit) {
    var arrayIngresos = [],
        arrayDeducciones = [],
        descripcionPlanilla = inputDescripcionPlanilla.val(),
        frecuenciaDias = inputFrecuenciaEnDias.val();
    if (listaCatalogoIngresos(arrayIngresos), listaCatalogoDeducciones(arrayDeducciones), edit) {
        let idPlanilla = inputIdPlanilla.val();
        mostrarSpinner($("#btnConfirmacionEdit"), cargandoEditar), _ajax({
            id: idPlanilla,
            ...
            "bien" == data ? (iziToast.success({
                title: "Exito",
                message: "¡El registro se editó de forma exitosa!"
            }), location.href = baseUrl + "/Index") : (iziToast.error({
                title: "Error",
                message: "No se editó el registro, contacte al administrador"
            }), ocultarSpinnerSpinner($("#btnConfirmacionEdit"), cargandoEditar))
        }, enviar => { })
    } else mostrarSpinner($("#btnGuardarCatalogoDePlanillasIngresosDeducciones"), cargandoCrear), _ajax({
        catalogoDePlanillas: [descripcionPlanilla, frecuenciaDias],
        catalogoIngresos: arrayIngresos,
        catalogoDeducciones: arrayDeducciones,
        checkRecibeComision: checkRecibeComision.checked
...
function listaCatalogoDeducciones(arrayDeducciones) {
    $('#catalogoDeDeducciones #tblCatalogoDeducciones tbody tr td input[type="checkbox"].i-checks').each((function (index, val) {
        let checkIdDedu, arrDedu, currentCheckboxIdDedu = $(val).attr("id").split("-")[1],
            isCheckedDedu;
        !0 === $("#catalogoDeDeducciones #tblCatalogoDeducciones tbody tr td #check-" + currentCheckboxIdDedu).is(":checked", !0) && arrayDeducciones.push(currentCheckboxIdDedu)
    }))
}

function listaCatalogoIngresos(arrayIngresos) {
    $('#catalogoDeIngresos input[type="checkbox"].i-checks').each((function (index, val) {
        let checkId, arr, currentCheckboxIdIngreso = $(val).attr("id").split("-")[1],
            isChecked;
        $("#catalogoDeIngresos #check-" + currentCheckboxIdIngreso).is(":checked", !0) && arrayIngresos.push(currentCheckboxIdIngreso)
    }))
}

function listaCatalogoDeduccionesFalse() {
    var hayUnoFalso = !0;
    return $('#catalogoDeDeducciones table tbody tr td input[type="checkbox"].i-checks').each((function (index, val) {
        let checkIdDedu, arrDedu, currentCheckboxIdDedu = $(val).attr("id").split("-")[1],
            isCheckedDedu;
        $("#catalogoDeDeducciones tbody tr td input#check-" + currentCheckboxIdDedu).prop("checked") || (hayUnoFalso = !1)
    })), hayUnoFalso
}

function listaCatalogoIngresosFalse() {
    var hayUnoFalso = !0;
    return $('#catalogoDeIngresos table tbody tr td input[type="checkbox"].i-checks').each((function (index, val) {
        let checkId, arr, currentCheckboxIdIngreso = $(val).attr("id").split("-")[1],
            isChecked;
        $("#catalogoDeIngresos tbody tr td input#check-" + currentCheckboxIdIngreso).prop("checked") || (hayUnoFalso = !1)
    })), hayUnoFalso
}
...
const Activar = '           <button type="button" class="btn btn-primary btn-xs" id="btnDetalleCatalogoDeducciones">Detalles</button>\n\t\t\t\t\t\t\t<button type="button" class ="btn btn-default btn-xs" id="btnActivar">Activar</button>\n\t\t\t\t\t\t\t',
    DetallesEditar = '\n\t\t\t\t\t\t\t<button type="button" class="btn btn-primary btn-xs" id="btnDetalleCatalogoDeducciones">Detalles</button>\n\t\t\t\t\t\t\t<button type="button" class="btn btn-default btn-xs" id="btnEditarCatalogoDeducciones">Editar</button>\n\t\t\t\t\t\t\t<button type="button" class="btn btn-danger btn-xs" id="Inactivar">Inactivar</button>\n\t\t\t\t\t\t\t';

function listar() {
    obtenerIdDetallesEditar("#tblCatalogoPlanillas tbody", table = $(".dataTables-example").DataTable({
        destroy: !0,
        ajax: {
            method: "GET",
            url: urlSinElIndex + "/getPlanilla",
            contentType: "application/json; charset=utf-8",
            dataType: "json"
        },
        columns: [{
            ...
            data: "activoAdmin",
            render: function (data) {
                return 0 == data.activo && 1 == data.esAdmin ? Activar : data.activo && data.esAdmin ? DetallesEditar : data.activo || data.esAdmin ? DetallesEditar : Activar
            }
        }],
        language: {
            sProcessing: spinner(),
...
function listarCatalogos() {
    ...
    function seleccionarCheckbox_CatalogoIngresos() {
        ...
        catalogoIngresosChangeCheckbox.onchange = function () {
            ...
            catalogoIngresosChangeCheckbox.checked ? (catalogoIngresosInputs.iCheck("check"), seleccionarTodosLosIngresos.html(seleccionarDeseleccionar.replace("Seleccionar", "Deseleccionar"))) : (catalogoIngresosInputs.iCheck("uncheck"), seleccionarTodosLosIngresos.html(seleccionarDeseleccionar.replace("Deseleccionar", "Seleccionar")))
        }, $(catalogoIngresosInputs).on("ifChecked", () => {
            listaCatalogoIngresosFalse() && ($("#checkSeleccionarTodosIngresos").is(":checked") || $("#checkSeleccionarTodosIngresos").prop("checked", !0))
        }), listaCatalogoIngresosFalse() && $("#checkSeleccionarTodosIngresos").click()
    }
    estaEnEditar() > 1 ? (urlFetchData += "false&id=" + URLactual.substr(URLactual.search(/Edit/i) + "Edit".length + 1), table = $(".tbl-catalogos").DataTable({
        language: {
            paging: !1,
            sProcessing: spinner(),
            sLengthMenu: "_MENU_",
        ...
        initComplete: function (settings, json) {
            ...
            catalogoIngresosChangeCheckbox.onchange = function () {
                ...
                validacionCatalogoIngresos.hide(), listaCatalogoIngresosFalse() && ($("#checkSeleccionarTodosIngresos").is(":checked") || $("#checkSeleccionarTodosIngresos").click())
            }), listaCatalogoIngresosFalse() && $("#checkSeleccionarTodosIngresos").click()
        }
        ...
        initComplete: function (settings, json) {
            ...
            $("#noAplica").on("ifChecked", () => {
                ...
                seleccionarTodasLasDeducciones.is(":checked") && seleccionarTodasLasDeducciones.click()
            }), $(catalogoDeduccionesInputs).on("ifChecked", () => {
                $("#noAplica").iCheck("uncheck"), listaCatalogoDeduccionesFalse() && (checkSeleccionarTodasLasDeducciones.is(":checked") || checkSeleccionarTodasLasDeducciones.click())
                ...
                catalogoDeduccionesChangeCheckbox.checked ? (catalogoDeduccionesInputs.iCheck("check"), seleccionarTodasLasDeducciones.html(seleccionarDeseleccionar.replace("Seleccionar", "Deseleccionar"))) : (seleccionarTodasLasDeducciones.html(seleccionarDeseleccionar.replace("Deseleccionar", "Seleccionar")), catalogoDeduccionesInputs.iCheck("uncheck"))
            }, listaCatalogoDeduccionesFalse() && checkSeleccionarTodasLasDeducciones.click()
        }
        ...
        initComplete: function (settings, json) {
            ...
            $("#noAplica").on("ifChecked", () => {
                ...
                validacionCatalogoDeducciones.hide(), $("#noAplica").iCheck("uncheck"), listaCatalogoDeduccionesFalse() && (checkSeleccionarTodasLasDeducciones.is(":checked") || checkSeleccionarTodasLasDeducciones.click())
            }), catalogoDeduccionesChangeCheckbox.onchange = function () {
                const seleccionarTodasLasDeducciones = $("#seleccionarTodasLasDeducciones");
                let seleccionarDeseleccionar = seleccionarTodasLasDeducciones.html();
                catalogoDeduccionesChangeCheckbox.checked ? (catalogoDeduccionesInputs.iCheck("check"), seleccionarTodasLasDeducciones.html(seleccionarDeseleccionar.replace("Seleccionar", "Deseleccionar"))) : (seleccionarTodasLasDeducciones.html(seleccionarDeseleccionar.replace("Deseleccionar", "Seleccionar")), catalogoDeduccionesInputs.iCheck("uncheck"))
...
function obtenerIdDetallesEditar(tbody, table) {
    "//" == pathname && (pathname = baseUrl), $(document).on("click", "button#btnEditarCatalogoDeducciones", (function () {
        var data = table.row($(this).parents("tr")).data();
        location.href = urlSinElIndex + "/Edit/" + data.idPlanilla
        ...
        localStorage.setItem("id1", table.row($(this).parents("tr")).data().idPlanilla), localStorage.setItem("element1", JSON.stringify($(this).parents("tr"))), $("#InactivarCatalogoDeducciones").modal({
            backdrop: "static",
            keyboard: !1
        })
    })), $(tbody).on("click", "button#btnActivar", (function () {
        localStorage.setItem("id", table.row($(this).parents("tr")).data().idPlanilla), localStorage.setItem("element", JSON.stringify($(this).parents("tr"))), $("#frmActivarCatalogoPlanilla").modal({
            backdrop: "static",
            keyboard: !1
        });
...
function getIngresos(data) {
    ...
    return $.each(data.ingresos, (function (index, val) {
...
function getDeducciones(data) {
    ...
    return $.each(data.deducciones, (function (index, val) {
...
$(document).ready(() => {
    ...
    }), estaEnCrear() > 1 && ($('input[type="checkbox"]').prop("checked", !1), inputDescripcionPlanilla.val(""), inputFrecuenciaEnDias.val("")), inputDescripcionPlanilla.blur((function () {
        "" != $(this).val().trim() ? (validacionDescripcionPlanilla.hide(), asteriscoDescripcionPlanilla.removeClass("text-danger")) : (asteriscoDescripcionPlanilla.addClass("text-danger"), validacionDescripcionPlanilla.show())
    })), inputFrecuenciaEnDias.blur((function () {
        "0" != inputFrecuenciaEnDias.val() || inputFrecuenciaEnDias.val() > 0 || "" != inputFrecuenciaEnDias.val() ? (validacionFrecuenciaDias.hide(), asteriscoFrecuenciaPago.removeClass("text-danger")) : (validacionFrecuenciaDias.show(), asteriscoFrecuenciaPago.addClass("text-danger"))
    }))
}), $(document).on("click", "td.details-control", (function () {
    var tr = $(this).closest("tr"),
        row = table.row(tr),
        detail;
    row.child.isShown() ? (row.child.hide(), tr.removeClass("shown")) : (row.child([spinner()]).show(), tr.addClass("shown"), localStorage.setItem("idDatatableChild", row.data().idPlanilla), obtenerDetalles(row.data().idPlanilla, data => {
        row.child([getIngresos(data) + getDeducciones(data)]).show(), tr.addClass("shown"), detail = null, detail = new $("table tbody tr td #child-" + localStorage.getItem("idDatatableChild")).DataTable({
            destroy: !0,
            language: {
                paging: !1,
                sProcessing: spinner(),
    ...
    table.search("").draw(), table2.search("").draw();
    let arrayIngresos = [],
        arrayDeducciones = [],
        descripcionPlanilla = inputDescripcionPlanilla.val(),
        frecuenciaDias = inputFrecuenciaEnDias.val();
    listaCatalogoIngresos(arrayIngresos), listaCatalogoDeducciones(arrayDeducciones), verificarCampos(descripcionPlanilla, frecuenciaDias, arrayIngresos, arrayDeducciones) && crearEditar(!1)
    ...
    let arrayIngresos = [],
        ...
        descripcionPlanilla = inputDescripcionPlanilla.val(),
        frecuenciaDias = inputFrecuenciaEnDias.val();
    listaCatalogoIngresos(arrayIngresos), listaCatalogoDeducciones(arrayDeducciones), verificarCampos(descripcionPlanilla, frecuenciaDias, arrayIngresos, arrayDeducciones) && $("#modalConfirmacionEdit").modal({
        backdrop: "static",
        keyboard: !1
    })
    ...
    $.ajax({
        ...
        if (0 == data) $("#InactivarCatalogoDeducciones").modal("hide"), iziToast.error({
            title: "Error",
            message: "No tienes permisos para realizar esta acción"
        });
        else {
            var id = localStorage.getItem("id1");;
            mostrarSpinner($("#btnInactivarPlanilla"), cargandoEliminar), _ajax({
                id: id
            }, "/CatalogoDePlanillas/Delete", "POST", data => {
                "bien" == data.response ? (iziToast.success({
                    title: "Exito",
                    message: "¡El registro se inactivó de forma exitosa!"
                }), $("#InactivarCatalogoDeducciones").modal("hide"), table.clear(), table.rows.add(data.data).draw()) : iziToast.error({
                    title: "Error",
...
$(document).on("click", "#btnActivarCatatalogoPlanilla", () => {
    $.ajax({
        ...
        if (0 == data) $("#frmActivarCatalogoPlanilla").modal("hide"), iziToast.error({
            title: "Error",
            message: "No tienes permisos para realizar esta acción"
        });
        else {
            let id = localStorage.getItem("id");
            mostrarSpinner($("#btnActivarCatatalogoPlanilla"), $("#cargandoActivar")), _ajax({
                id: id
            }, "/CatalogoDePlanillas/ActivarPlanilla", "POST", data => {
                "bien" == data.response ? (iziToast.success({
                    title: "Éxito",
                    message: "¡El registro se activó de forma exitosa!"
                }), $("#frmActivarCatalogoPlanilla").modal("hide"), table.clear(), table.rows.add(data.data).draw()) : iziToast.error({
                    title: "Éxito",
